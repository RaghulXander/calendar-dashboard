...
import { CalendarType } from "../../stores/models/calendar.model";
	...
	const displayDate = useMemo(() => {
		switch (calendarType) {
			case LayoutTypes.DAY: {
				return <span>{currentDay.name} {currentMonth.name} {currentYear.name}</span>
			}
			case LayoutTypes.WEEK: {
				return <span>{currentMonth.name} {currentYear.name}</span>
			}
		...
		<div className={styles["header"]}>
			<div className={styles["left-sec"]}>
				...
				<div className={styles["nav-actions"]}>
					<button title="previous" onClick={calendarActions.updatePrevious}><SvgIconLftArrow /></button>
					<button title="next" onClick={calendarActions.updateNext}><SvgIconRgtArrow /></button>
				</div>
				<h2 className={styles["date-display"]}>{displayDate}</h2>
			</div>
			<div className={styles["right-sec"]}>
				<div className={styles["filter_by"]}>
					<Dropdown options={Object.values(LayoutTypes)} value={calendarType} handleSelect={(value) => calendarActions.updateCalendarType(value as CalendarType)} />
				</div>
			</div>
			<div className={styles["tools"]}>
				<div className={styles["time"]}><IconCountdown /><span>Yet to Respond</span></div>
				<span className={styles["print"]}><IconPrint /></span>
			</div>
		</div>
	)
};
